...
// ===== PORTAL SSO – SID CONFIG =====
const PORTAL_SESSION_SHEET = 'PORTAL_SESSIONS';
const SID_TTL_SECONDS = 24 * 60 * 60; // 24 giờ

/*****************************************************
 * PORTAL SSO – PHÁT HÀNH SID TỪ JWT
 *****************************************************/
/*****************************************************
 * JWT – VERIFY (HS256) dùng cho issue-sid, v.v.
 *****************************************************/

/**
 * Giải mã base64url -> string UTF-8
 */
...
/**
 * Chuẩn hoá token (phòng trường hợp có "Bearer ", bị encode, v.v.)
 */
...
/**
 * verifyJWT – kiểm tra chữ ký + hạn dùng
 * Trả về { ok:true, payload } hoặc { ok:false, error }
 */
function verifyJWT(token) {
  try {
    ...
    // Tính lại chữ ký với JWT_SECRET
    var rawSig = Utilities.computeHmacSha256Signature(
      data,
      JWT_SECRET
    );
    var expectedSigB64 = Utilities.base64EncodeWebSafe(rawSig).replace(/=+$/, '');
    ...
    // Decode payload
    var payloadStr = base64UrlDecodeToString_(payloadB64);
...
/**
 * Lấy sheet PORTAL_SESSIONS, tự tạo nếu chưa có
 */
function getPortalSessionSheet_() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID); // file ACCOUNTS đang dùng
  let sh = ss.getSheetByName(PORTAL_SESSION_SHEET);
  if (!sh) {
    sh = ss.insertSheet(PORTAL_SESSION_SHEET);
  }
  if (sh.getLastRow() === 0) {
    sh.appendRow(['SID', 'PAYLOAD', 'EXP', 'APP_ID', 'CREATED_AT']);
  }
  return sh;
}

/**
 * Phát hành SID mới từ JWT hợp lệ
 * data: { token, appId }
 */function normalizeToken_(raw) {
  if (!raw) return '';
  let t = String(raw).trim();

  // Nếu lỡ lưu dạng "Bearer xxxxx"
  if (t.startsWith('Bearer ')) {
    t = t.slice(7).trim();
  }

  // Nếu token bị bọc bởi dấu nháy do JSON/stringify
  if ((t.startsWith('"') && t.endsWith('"')) ||
      (t.startsWith("'") && t.endsWith("'"))) {
    t = t.slice(1, -1);
  }

  // Nếu lỡ encodeURIComponent trước đó
  try {
    const dec = decodeURIComponent(t);
    // dạng JWT phải có 3 phần ngăn bởi dấu chấm
...
function handleIssueSid(data) {
  let token = data && data.token ? String(data.token) : '';
  const appId = data && data.appId ? String(data.appId) : 'ANY';
  ...
  // verifyJWT(token) đã có sẵn trong Auth Service
  const v = verifyJWT(token);
  if (!v || !v.ok) {
    return { ok: false, error: 'INVALID_TOKEN', debug: v };
  }

  const payload = v.payload || {};
  const nowSec = Math.floor(Date.now() / 1000);
  const expSec = nowSec + SID_TTL_SECONDS;

  const sid = Utilities.getUuid().replace(/-/g, '');

  const sh = getPortalSessionSheet_();
  sh.appendRow([
    sid,
...
/**
 * Đọc session từ SID – dùng chung cho các app khác nếu cần
 */
function getPortalSession_(sid, appId) {
  if (!sid) return { ok: false, error: 'MISSING_SID' };
  ...
  for (let i = 1; i < values.length; i++) {
    const [sidCol, payloadCol, expCol, appCol] = values[i];

    if (sidCol === sid) {
      if (expCol && expCol < nowSec) {
        return { ok: false, error: 'SID_EXPIRED' };
      }
      if (appId && appId !== 'ANY' && appCol && appCol !== 'ANY' && appCol !== appId) {
        return { ok: false, error: 'WRONG_APP' };
      }
      try {
        const payload = JSON.parse(payloadCol);
        return { ok: true, user: payload, exp: expCol, appId: appCol };
      } catch (e) {
        return { ok: false, error: 'BAD_PAYLOAD' };
      }
    }
  }
  return { ok: false, error: 'SID_NOT_FOUND' };
}

/**
 * (Tuỳ chọn) Dọn session hết hạn – anh có thể gọi bằng Trigger time-based
 */
function cleanupPortalSessions_() {
  const sh = getPortalSessionSheet_();
  const values = sh.getDataRange().getValues();
  const nowSec = Math.floor(Date.now() / 1000);
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'issue-sid':        return jsonResponse(handleIssueSid(data));
